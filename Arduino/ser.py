...
while(opcion):
    ...
    if (not opcion):
        break
    if (opcion):
        user = int(input("Ingrese su numero de usuario: "))
        while(len(hrValues)!=100):
            try:
                ...
                clear()
                percent += "|"
                print(f"{len(hrValues)}%" + percent, end= "")
                #print(line)
            except:
                continue
        if (opcion):
            spo2 = calc_hr_and_spo2(irValues, redValues)
            hr = round(analisisHR2(hrValues, miliValues, 1))
            print()
            print("SPO2:",spo2)
            print("HR:",hr)
            hrValues=[]
            ...
            if (spo2[3]):
                fecha = datetime.now()
                add_register_ritmo(user, hr, fecha)
                add_register_oxigeno(user, spo2[2], fecha)
    else:
        name = input("Nombre: ")
        age = int(input("Edad: "))
        weight = int(input("Peso: "))
        add_user(name, age, weight)
